...
import { confirm, printOrder } from "./prompt";
	...
	if (chainId) {
		...
		chainName =
			chainId === ChainIds.MAINNET
				? chalk.green(selectedChain)
				: chalk.cyan(selectedChain);
	}
...
export async function getChainId(ctx: any): Promise<string> {
	...
	if (!ChainIds[chainId])
		throw `Chain ${chainId} not supported. Change with chain command.`;
...
export function getAtomicValue(value: string, token: string, metadata: any) {
	return new BigNumber(value).multipliedBy(
		new BigNumber(10).pow(metadata.byAddress[token.toLowerCase()].decimals),
	);
}
...
export async function getBalanceChanges(
	...
	let { signerToken, signerAmount, senderWallet, senderToken, senderAmount } =
		order;
	if (order.signer) {
	...
	const signerTokenBalance = await new ethers.Contract(
		signerToken,
		IERC20.abi,
		wallet,
	).balanceOf(senderWallet);
	const senderTokenBalance = await new ethers.Contract(
		senderToken,
		IERC20.abi,
		wallet,
	).balanceOf(senderWallet);

	const signerTokenBalanceDecimal = getDecimalValue(
		signerTokenBalance.toString(),
		signerToken,
		metadata,
	);
	const senderTokenBalanceDecimal = getDecimalValue(
		senderTokenBalance.toString(),
		senderToken,
		metadata,
	);
	const signerTokenChangeDecimal = getDecimalValue(
		signerAmount,
		signerToken,
		metadata,
	);
	const senderTokenChangeDecimal = getDecimalValue(
		senderAmount,
		senderToken,
		metadata,
	);
	const newSignerTokenBalance = getDecimalValue(
		signerTokenBalance.add(signerAmount).toString(),
		signerToken,
		metadata,
	);
	const newSenderTokenBalance = getDecimalValue(
		senderTokenBalance.sub(senderAmount).toString(),
		senderToken,
		metadata,
	);
...
export function getByLowestSenderAmount(results) {
	...
	for (let j = 1; j < results.length; j++) {
		if (
			new BigNumber(results[j].order.sender.amount).lt(
				lowest.order.sender.amount,
			)
		) {
			lowest = results[j];
		}
...
export function getByHighestSignerAmount(results) {
	...
	for (let j = 1; j < results.length; j++) {
		if (
			new BigNumber(results[j].order.signer.amount).gt(
				highest.order.signer.amount,
			)
		) {
			highest = results[j];
		}
...
export async function handleResponse(
	...
	if (order) {
		...
		if (!(await printOrder(ctx, request, order, wallet, metadata))) {
			ctx.log(
				`${chalk.yellow("Unable to take")}: your token balance is insufficient.\n\n`,
			);
		} else if (
			await confirm(
				ctx,
				...
				"swapLight",
				{
					...
					signerAmount: `${order.signerAmount} (${chalk.cyan(
						toDecimalString(
							order.signerAmount,
							metadata.byAddress[request.signerToken.address].decimals,
						),
					)})`,
					...
					senderAmount: `${order.senderAmount} (${chalk.cyan(
						toDecimalString(
							order.senderAmount,
							metadata.byAddress[request.senderToken.address].decimals,
						),
					)})`,
				},
				chainId,
		...
		if (errors.length) {
			...
			ctx.log(
				`${chalk.yellow("No servers found.")} Protocol: RequestForQuoteERC20\n`,
			);
		}
	}
}
